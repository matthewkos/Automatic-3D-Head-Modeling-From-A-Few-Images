...
class PRN:
	...
	def __init__(self, is_mtcnn = False, prefix = '.'):
		...
		# uv file
		self.uv_kpt_ind = np.loadtxt(os.path.join(prefix, __package__, 'Data/uv-data/uv_kpt_ind.txt')).astype(np.int32) # 2 x 68 get kpt
		self.face_ind = np.loadtxt(os.path.join(prefix, __package__, 'Data/uv-data/face_ind.txt')).astype(np.int32) # get valid vertices in the pos map
		self.triangles = np.loadtxt(os.path.join(prefix, __package__, 'Data/uv-data/triangles.txt')).astype(np.int32) # ntri x 3
	...
	def process(self, input, image_info = None):
		...
		if image_info is not None:
			...
			d = detected_faces[0]['box'] ## only use the first detected face (assume that each input image only contains one face)
			left = d[0]; right = d[0]+d[2]; top = d[1]; bottom = d[1]+d[3];
			old_size = (right - left + bottom - top)/2
